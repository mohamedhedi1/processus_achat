...
function Steppers(props) {
 ...
 console.log(props.listApprouvation)
  ...
  const handleClickOpen = () => {
    if(fermer===true){setFermer(false)}
    setOpen(true);
  };
  const handleClose = () => {
    setOpen(false);
  ...
  return (
   ...
   <Stepper activeStep={listApps.length-1}>
     {listApps.map((listApp)=>
     {
       ...
       {!fermer && <Modal_stepper handleClose={handleClose} setFermer={setFermer} setOpen={setOpen} open={open}  AllFiles={props.AllFiles} listApp={listApp}></Modal_stepper>}   
       ...
       {return(
        <Step >
          <Modal_stepper handleClose={handleClose} setOpen={setOpen} open={open} AllFiles={props.AllFiles} listApp={listApp}></Modal_stepper>
